...
import { coachBookingValidationSchema } from "../BookingForm/validationSchema";
...
const AddSingleBooking = (props: IProps) => {
    const data = props?.booking

    const [request] = useAddBookingMutation();
    const [fetchBookingPrice] = useFetchBookingPriceMutation()
    const [updateBooking] = useEditBookingMutation()

    const [fetchCoachesReqeust, fetchCoachesResponse] = useLazyFetchCoachesQuery()
    ...
    const formik = useFormik({
        initialValues: {
            ...
            payment_actual_amount: data?.payment_actual_amount || "",
            ...
            coach_id: data?.coach?.id || undefined,
            is_coaching_booking: true
        },
        validationSchema: coachBookingValidationSchema,
        onSubmit: (values) => {
    ...
    // Add this function to handle coach selection
    const handleCoachChange = (event: React.ChangeEvent<{ value: unknown }>) => {
        const coachId = event.target.value as number;
        const selectedCoach = fetchCoachesResponse?.data?.data?.find(coach => coach.id === coachId);
        if (selectedCoach) {
            formik.setFieldValue("coach_id", coachId);
            formik.setFieldValue("coach_name", selectedCoach.name);
        }
    };
    ...
    return (
        <form onSubmit={formik.handleSubmit}>
            <Grid container spacing={3}>
                <Grid item xs={12}>
                    <CustomFormLabel htmlFor="coach_id">Coach</CustomFormLabel>
                    <CustomSelect
                        id="coach_id"
                        fullWidth
                        value={formik.values.coach_id || ""}
                        onChange={handleCoachChange}
                        error={formik.touched.coach_id && Boolean(formik.errors.coach_id)}
                    >
                        <MenuItem value="">
                            <em>Select a coach</em>
                        </MenuItem>
                        {fetchCoachesResponse?.data?.data?.map((coach) => (
                            <MenuItem key={coach.id} value={coach.id}>
                                <Box display="flex" alignItems="center">
                                    <Avatar src={coach.image} alt={coach.name} sx={{ mr: 2, width: 32, height: 32 }} />
                                    {coach.name}
                                </Box>
                            </MenuItem>
                        ))}
                    </CustomSelect>
                    {formik.touched.coach_id && formik.errors.coach_id && (
                        <Typography variant="caption" color="error">
                            {formik.errors.coach_id}
                        </Typography>
                    )}
                </Grid>
